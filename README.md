...
│   ├── kb_matches.csv   # Матчи FAQ-поиска (для улучшения базы знаний)
...
- Записаться онлайн (открывает форму YClients)

Каждый экран содержит кнопки навигации «⬅️ Назад» и «🏠 Главное меню».
...
### Трёхуровневый роутинг ответов
1. **Статические карточки** — мгновенный ответ для всех кнопок меню и ключевых текстовых запросов (цены, адрес, магазин, преимущества). Без GPT.
2. **FAQ-поиск по базе знаний** — гибридный поиск (SequenceMatcher + токены + n-граммы). Без GPT.
3. **GPT-4o-mini** — только для сложных консультаций, подбора услуг и нестандартных вопросов.
...
- Кэш ответов FAQ-поиска логируется в `logs/kb_matches.csv` для анализа качества базы знаний
...
- **Python 3.11**
- **python-telegram-bot 22.5** — Telegram Bot API (polling, inline-кнопки, job queue)
...
### v1.1 (2026-03)
- **fix**: исправлен `_estimate_booking_amount` — `("деконтам")` и `("подкапот")` были строками вместо кортежей, аналитика выручки ломалась
- **fix**: `menu_self` добавлен в `SUB_MENUS` — кнопки подменю самообслуживания теперь отображаются
- **fix**: удалён мёртвый блок кода в `handle_message` (lines 2727–2804) — статические карточки для текстовых запросов (цены, адрес, магазин) теперь работают
- **fix**: исправлена мутация `query.data` в `button_callback` (несовместима с ptb v22)
- **fix**: кнопка «⬅️ Назад» в текстовом чате теперь использует `menu_back_text` — история диалога восстанавливается корректно
- **fix**: `send_answer` теперь уважает `NO_CTA_CALLBACKS` — адрес, портфолио и другие информационные блоки не получают лишний продающий хвост
- **fix**: добавлена навигация к ответам `sub_shop_*` в `button_callback`
- **fix**: `rated` сбрасывается при повторном принятии согласия (`consent_accept`)
- **fix**: добавлены `sub_self_*` в `BACK_TARGETS` — кнопка «Назад» появляется в подменю самообслуживания
- **fix**: удалены дублирующие кнопки в `SHOP_KEYBOARD`
