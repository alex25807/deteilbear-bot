...
WASH_KEYBOARD = InlineKeyboardMarkup([
    [
        InlineKeyboardButton("🚿 Двухфазная мойка (2 500 ₽)", callback_data="sub_wash_twophase"),
    ],
    [
        InlineKeyboardButton("🧽 Комплексная мойка (8 500 ₽)", callback_data="sub_wash_complex"),
    ],
    [
        InlineKeyboardButton("📅 Записаться онлайн", web_app=WebAppInfo(url="https://n1024167.yclients.com/")),
    ],
...
MENU_PROMPTS = {
    ...
    "sub_wash_twophase": "Расскажите подробно про двухфазную мойку кузова — что входит, сколько стоит?",
...
STATIC_MENU_ANSWERS = {
    "menu_services": (
        "Подберем формат ухода под Вашу задачу и бюджет. Обычно клиенты выбирают один из 3 сценариев:\n"
        "• Быстро освежить авто — мойка и поддерживающий уход\n"
        "• Сделать «как новый» — комплексная мойка/салон/полировка\n"
        "• Защитить надолго — керамика или полиуретановая пленка\n\n"
        ...
        "Выберите нужный раздел ниже — и сразу покажу, что входит, сроки и ориентир по стоимости ⬇️"
    ),
    "menu_self": (
    ...
    "sub_protection": (
        ...
        "(легкие: 3–12 месяцев, профессиональные: 2–3 года).\n"
        "По пленкам доступны варианты NXTZEN / SKINCARS / Crystal Ultra Gloss.\n"
        "Пленки защищают от сколов, царапин, пескоструя и реагентов."
    ),
    "sub_decon": (
        "Деконтаминация ЛКП — от 6 000 ₽, от 2 часов.\n"
        "Удаляем металлические вкрапления, битум и стойкие органические загрязнения.\n"
        "Перед услугой обязательна быстрая или комплексная мойка."
    ),
    "sub_engine_wash": (
        "Мойка подкапотного пространства — от 9 000 ₽, от 4 часов.\n"
        "Что делаем: изоляция чувствительных компонентов, деликатная мойка, "
        "продувка и обработка защитным консервантом."
    ),
    "sub_interior": (
    ...
    "sub_glass": (
        ...
        "Что входит: очистка/обезжиривание и профессиональная установка.\n"
...
ADVANTAGES_ANSWER = (
    ...
    "1️⃣ Профессионализм — наши мастера имеют большой опыт работы "
    "с разными марками и сложными задачами\n"
    "2️⃣ Качество — используем проверенную профессиональную автохимию "
    "OPT / Optimum, IK и другие сильные бренды\n"
    "3️⃣ Удобство — доступны и услуги мастеров, и пост самообслуживания 24/7\n"
    "4️⃣ Комфорт — тёплое помещение, кондиционирование, зона отдыха, "
    "кофе, чай, лёгкие закуски, Wi-Fi и парковка ☕\n"
    "5️⃣ Честный подход — все составы тестируем на реальных задачах "
    "в нашей студии, а не просто продаём «с витрины» ✅\n\n"
    "Записаться можно по кнопке ниже ⬇️"
)

PRICES_ANSWER = (
    "Чтобы сориентироваться быстрее, вот что чаще выбирают:\n"
    "• Быстрый уход — детейлинг-мойка от 3 000 ₽\n"
    "• Комплексный уход — детейлинг-мойка Bearlake от 8 000 ₽\n"
    "• Восстановление и блеск — полировка от 25 000 ₽\n"
    "• Долгая защита — керамика от 5 000 ₽ / пленки от 85 000 ₽\n\n"
    ...
    "Если хотите, сориентирую по оптимальному набору услуг под ваш бюджет и задачу.\n"
    "Записаться можно по кнопке ниже ⬇️"
)
